...
static int playerConstituteTeam(int id)
{
    ...
    {
        // if there are enough players to form team -> this player will be captain and form team
        ...
        {
            // change its state to WAITING_TEAM
    ...
    // if player is not captain -> wait for captain to call him; return team id; aknowledge registration
    if (sh->fSt.st.playerStat[id] == WAITING_TEAM)
    {
        // wait for captain to call him
        ...
        // return the synchronised value of team id
        ret = sh->fSt.teamId;

        // aknowledge registration
    ...
    else if (sh->fSt.st.playerStat[id] == FORMING_TEAM) // if player is captain -> tell the ref one of the teams has been formed
    {
 ...
 *  \brief player waits for referee to start match
 *
 *  The player updates its state and waits for referee to end match. ->> this should be start match
 *  The internal state should be saved.
 *
 *  \param id   player id
...
static void waitReferee(int id, int team)
{
    ...
    // decide which team the player is waiting for
...
static void playUntilEnd(int id, int team)
{
    ...
    // decide which team the player is playing for
